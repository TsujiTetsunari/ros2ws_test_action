...
public:
  ...
  {
    wait_server_ =
      rclcpp_action::create_server<lecture_msgs::action::WaitTask>(
...
private:
  rclcpp_action::GoalResponse handleGoal(
    ...
    std::shared_ptr<const lecture_msgs::action::WaitTask::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse handleCancel(
    const std::shared_ptr<rclcpp_action::ServerGoalHandle<lecture_msgs::action::WaitTask>> goal_handle)
  {
  ...
  void onIntervalTimer(void) {
    if (goal_handle_) {
      ...
      if (threshold_count <= counter_) {
        ...
        auto result = std::make_shared<lecture_msgs::action::WaitTask::Result>();
        result->message = "full count";
        goal_handle_->succeed(result);
        goal_handle_.reset();
      } else {
        auto feedback = std::make_shared<lecture_msgs::action::WaitTask::Feedback>();
  ...
  rclcpp_action::Server<lecture_msgs::action::WaitTask>::SharedPtr wait_server_;
  rclcpp::TimerBase::SharedPtr interval_timer_{nullptr};
  int counter_{0};

  std::shared_ptr<rclcpp_action::ServerGoalHandle<lecture_msgs::action::WaitTask>> goal_handle_;
};
